      ...
      <div className="space-y-1">
        {recentTransactions.length > 0 ? (
          recentTransactions.map((tx) => (
            <div
              key={tx._id || tx.id}
              className="flex items-center justify-between p-4 rounded-xl hover:bg-surface-container-high transition-colors group"
            >
              ...
              <div className="flex items-center gap-6">
                <span
                  className={`text-lg font-bold ${tx.type === 'income' ? 'text-secondary' : 'text-tertiary'}`}
                >
                  {tx.type === 'income' ? '+' : '-'}$
                ...
                <div className="flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                  <button
                    onClick={() => onEdit(tx)}
                    className="w-8 h-8 rounded-full flex items-center justify-center bg-surface-bright text-on-surface-variant hover:text-primary transition-colors"
                  >
                  ...
                  <button
                    onClick={() => handleDelete(tx._id || tx.id, tx.title)}
                    className="w-8 h-8 rounded-full flex items-center justify-center bg-surface-bright text-on-surface-variant hover:text-error transition-colors"
                  >
                    <span className="material-symbols-outlined text-sm">
                      delete
                    </span>
